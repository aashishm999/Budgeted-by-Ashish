...
export default function GroupView({ groupId, user, onBack, theme }: GroupViewProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribeMembers = onSnapshot(membersQuery, (snapshot) => {
      ...
      try {
        handleFirestoreError(error, OperationType.LIST, `groups/${groupId}/members`);
      } catch (e) {
        // Silent if expected
      }
    });
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!emailLookupSnap.exists()) {
        setInviteError('User not found. They must sign in to the app at least once to be discoverable.');
  ...
  const handleChat = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const systemPrompt = `You are a helpful financial assistant for a group expense tracker named "${group?.name}". 
        ...
        - Categories: ${Object.entries(categoryTotals).map(([cat, amount]) => `${cat}: ${CURRENCY_SYMBOL}${formatCurrency(amount)}`).join(', ')}
        - Recent Expenses: ${expenses.slice(0, 5).map(e => `${e.description}: ${CURRENCY_SYMBOL}${formatCurrency(e.amount)}`).join(', ')}`;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-4 lg:gap-6 mb-12">
            ...
            {group.maxBudget && (
              <div className="mt-6">
                ...
                <p className="text-[10px] text-zinc-500 mt-2 font-medium">
                  ${CURRENCY_SYMBOL}{formatCurrency(currentBudgetSpent)} of ${CURRENCY_SYMBOL}{formatCurrency(group.maxBudget)}
                </p>
              </div>
            )}
          </div>
        ...
        >
          ...
          <div className="relative">
            ...
            >
              {CURRENCY_SYMBOL}{formatCurrency(perPerson)}
            </p>
        ...
        >
          ...
          <div className="relative">
            ...
            >
              {CURRENCY_SYMBOL}{formatCurrency(Math.abs(balance))}
            </p>
          </div>
        </button>
      </div>
              ...
              <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                <LineChart data={lineData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                  ...
                  <Tooltip 
                    ...
                    formatter={(value: number) => [`${CURRENCY_SYMBOL}${formatCurrency(value)}`, 'Spent']}
                  />
                  <Line 
                    type="monotone" 
                    dataKey="amount" 
            ...
            {pieData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                <PieChart>
                  ...
                  <Tooltip 
                    formatter={(value: number) => [`${CURRENCY_SYMBOL}${formatCurrency(value)}`, 'Total']}
                    contentStyle={{ 
                      borderRadius: '16px', 
                      border: 'none', 
                      boxShadow: '0 10px 25px -5px rgba(0,0,0,0.1)', 
        ...
        {isChatOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="relative w-full max-w-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[32px] shadow-2xl flex flex-col h-[80vh] overflow-hidden"
            >
              <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-zinc-900 dark:text-white font-display">Financial Assistant</h3>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
                {chatMessages.length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center text-center p-10">
                    ...
                    <h4 className="text-xl font-bold text-zinc-900 dark:text-white mb-2 font-display">How can I help you today?</h4>
                    <p className="text-sm text-zinc-500 dark:text-zinc-400 max-w-xs">
                      Ask me about your spending trends, budget status, or for tips on saving money.
                    </p>
                  </div>
                )}
                {chatMessages.map((msg, i) => (
                  <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] p-4 rounded-2xl ${msg.role === 'user' ? 'bg-indigo-600 text-white' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white'}`}>
              ...
              <form onSubmit={handleChat} className="p-6 border-t border-zinc-100 dark:border-zinc-800 shrink-0">
                <div className="relative">
                  <input
                    type="text"
                    value={chatInput}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-5xl sm:text-6xl font-bold text-zinc-900 dark:text-white font-display tracking-tight mb-2 break-all">
                {CURRENCY_SYMBOL}{formatCurrency(selectedStatDetails.amount)}
              </p>
