...
export default function GroupView({
  ...
  const [selectedStatDetails, setSelectedStatDetails] = useState<{
    title: string;
    amount: number;
    subtitle?: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubscribeGroup = onSnapshot(
      ...
      (doc) => {
        if (doc.exists()) {
          ...
          setGroup({ ...data, id: doc.id } as Group);
    ...
    const unsubscribeMembers = onSnapshot(
      ...
      (snapshot) => {
        setMembers(
          snapshot.docs.map(
            (doc) => ({ uid: doc.id, ...doc.data() }) as GroupMember,
          ),
        );
      },
      (error) => {
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingExpense) {
        await updateDoc(
          doc(db, "groups", groupId, "expenses", editingExpense.id),
          expenseData,
        );
      } else {
        await addDoc(
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const emailLookupRef = doc(db, "user_emails", email);
      const emailLookupSnap = await getDoc(emailLookupRef);

      if (!emailLookupSnap.exists()) {
        setInviteError(
          "User not found. They must sign in to the app at least once to be discoverable.",
        );
        setInviteLoading(false);
        return;
      }

      const userData = emailLookupSnap.data();
      const newUid = userData.uid;

      if (members.some((m) => m.uid === newUid)) {
      ...
      await setDoc(doc(db, "groups", groupId, "members", newUid), {
        ...
        displayName: userData.displayName || "New Member",
  ...
  const handleChat = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const systemPrompt = `You are a helpful financial assistant for a group expense tracker named "${group?.name}". 
        ...
        Help users understand their spending, suggest ways to save, or answer questions about the budget. 
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-8 mb-12">
        <div className="flex-1">
          ...
          <h1 className="text-4xl font-bold tracking-tight text-zinc-900 dark:text-white mb-3 font-display">
            {group.name}
          </h1>
          <p className="text-zinc-600 dark:text-zinc-300 max-w-2xl leading-relaxed font-medium">
        ...
        <div className="flex flex-wrap items-stretch gap-2 sm:gap-3 w-full md:w-auto">
          ...
          <button
            ...
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 sm:px-5 py-3 border border-indigo-200 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl text-sm font-bold text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 hover:shadow-lg hover:shadow-indigo-500/10 transition-all active:scale-95"
          >
          ...
          <button
            ...
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 sm:px-5 py-3 bg-linear-to-br from-indigo-600 to-violet-600 text-white rounded-2xl text-sm font-bold hover:from-indigo-700 hover:to-violet-700 hover:shadow-xl hover:shadow-indigo-500/40 transition-all disabled:opacity-50 shadow-lg shadow-indigo-500/20 active:scale-95"
          >
            {isAnalyzing ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-4 lg:gap-6 mb-12">
        <button
          onClick={() =>
            setSelectedStatDetails({
              title: "Total Group Spend",
              amount: totalSpent,
            })
          }
          className="text-left w-full bg-white dark:bg-zinc-900 p-6 md:p-5 lg:p-8 rounded-4xl border border-zinc-200 dark:border-zinc-800 shadow-xl shadow-zinc-200/50 dark:shadow-zinc-950/20 relative overflow-hidden group hover:scale-[1.02] active:scale-95 transition-all cursor-pointer"
        >
          ...
          <div className="relative">
            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em] mb-4 font-display">
              Total Group Spend
            </p>
            <p
              ...
              <div className="mt-6">
                <div className="flex justify-between text-[10px] font-bold uppercase mb-2 font-display">
                  <span className="text-zinc-500">
                    Budget ({group.budgetType})
                  </span>
                  <span
                    className={
                      currentBudgetSpent > group.maxBudget
                ...
                <div className="h-2 bg-zinc-100 dark:bg-white/10 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (currentBudgetSpent / group.maxBudget) * 100)}%`,
                    }}
                  />
                </div>
        ...
        <button
          onClick={() =>
            setSelectedStatDetails({
              title: "Your Share",
              amount: perPerson,
              subtitle: `${totalSpent > 0 ? ((userSpent / totalSpent) * 100).toFixed(0) : 0}% of total paid by you`,
            })
          }
          className="text-left w-full bg-white dark:bg-zinc-900 p-6 md:p-5 lg:p-8 rounded-4xl border border-zinc-200 dark:border-zinc-800 shadow-xl shadow-zinc-200/50 dark:shadow-black/20 relative overflow-hidden group hover:scale-[1.02] active:scale-95 transition-all cursor-pointer"
        >
          <div className="absolute top-0 right-0 w-32 h-32 bg-zinc-100 dark:bg-white/5 rounded-full -mr-16 -mt-16 transition-transform group-hover:scale-110" />
          <div className="relative">
            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em] mb-4 font-display">
            ...
            <p className="text-xs font-medium text-zinc-500 mt-4">
              {totalSpent > 0 ? ((userSpent / totalSpent) * 100).toFixed(0) : 0}
              % of total paid by you
            </p>
          </div>
        </button>
        <button
          onClick={() =>
            setSelectedStatDetails({
              title: balance >= 0 ? "You are owed" : "You owe",
              amount: Math.abs(balance),
            })
          }
          className="text-left w-full bg-white dark:bg-zinc-900 p-6 md:p-5 lg:p-8 rounded-4xl border border-zinc-200 dark:border-zinc-800 shadow-xl shadow-zinc-200/50 dark:shadow-black/20 relative overflow-hidden group hover:scale-[1.02] active:scale-95 transition-all duration-300 cursor-pointer"
        >
          <div
            className={`absolute top-0 right-0 w-32 h-32 rounded-full -mr-16 -mt-16 transition-transform group-hover:scale-110 ${balance >= 0 ? "bg-emerald-500/10" : "bg-red-500/10"}`}
          />
          <div className="relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
        ...
        <div
          className={`bg-white dark:bg-zinc-900 p-4 sm:p-8 rounded-[40px] border border-zinc-200 dark:border-zinc-800 shadow-xl shadow-zinc-200/50 dark:shadow-black/20 ${group.budgetType === "total" ? "lg:col-span-2" : ""}`}
        >
          <h3 className="text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-[0.15em] mb-8 flex items-center gap-2">
            ...
            {expenses.length === 0 ? (
              <div className="p-10 sm:p-20 text-center">
                ...
                <p className="text-zinc-500 dark:text-zinc-400 max-w-xs mx-auto text-sm">
                  Start tracking your shared expenses by adding your first
                  transaction.
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-center gap-4 sm:gap-5 min-w-0">
                      <div className="w-12 h-12 sm:w-14 sm:h-14 bg-zinc-50 dark:bg-zinc-800 rounded-2xl flex flex-col items-center justify-center text-zinc-400 dark:text-zinc-500 border border-zinc-100 dark:border-zinc-700 group-hover:bg-white dark:group-hover:bg-zinc-800 transition-colors shrink-0">
                        <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-tighter text-zinc-500">
                          {expense.date
                            .toDate()
                            .toLocaleDateString("en-US", { month: "short" })}
                        </span>
                        <span className="text-lg sm:text-xl font-bold leading-none text-zinc-900 dark:text-white">
                          {expense.date.toDate().getDate()}
                        </span>
                      </div>
                      <div className="min-w-0">
                        <p className="font-bold text-zinc-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors truncate">
                          {expense.description}
                        </p>
                        <div className="flex flex-wrap items-center gap-2 mt-1">
                          <span className="px-2 py-0.5 bg-zinc-100 dark:bg-zinc-800 rounded-full text-[9px] sm:text-[10px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider border border-zinc-200 dark:border-zinc-700 shrink-0">
                            {expense.category}
                          </span>
                          <span className="text-zinc-300 dark:text-zinc-700 hidden sm:inline shrink-0">
                            •
                          </span>
            ...
            <div className="space-y-6">
              {members.map((member) => (
                ...
                >
                  ...
                  {member.uid === group.createdBy && (
                    <span className="shrink-0 text-[9px] font-bold text-indigo-600 bg-indigo-50 dark:bg-indigo-500/10 px-2 py-0.5 rounded-full border border-indigo-100 dark:border-indigo-500/20 ml-2">
                      OWNER
                    </span>
                  )}
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleUpdateSettings} className="space-y-8">
                <div>
                  <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400 mb-6">
                    General Info
                  </h4>
                  ...
                  <div className="space-y-5">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setEditBudgetType(e.target.value as BudgetType)
                        }
                        className="w-full px-5 py-4 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium appearance-none dark:text-white"
                      >
            ...
            >
              ...
              {isAnalyzing ? (
                ...
                <div className="max-w-none">
                  <div className="bg-zinc-50 dark:bg-zinc-800 rounded-4xl p-8 border border-zinc-200 dark:border-zinc-700 analysis-content dark:text-zinc-300">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <p
                ...
                className="text-zinc-500 dark:text-zinc-400 text-sm mb-10 leading-relaxed"
              >
                This will permanently delete the group{" "}
                <strong>{group?.name}</strong> and all its expenses. This action
                cannot be undone.
              </p>
        ...
        {isChatOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="relative w-full max-w-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-4xl shadow-2xl flex flex-col h-[80vh] overflow-hidden"
            >
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
                {chatMessages.length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center text-center p-10">
                    ...
                    <h4 className="text-xl font-bold text-zinc-900 dark:text-white mb-2 font-display">
                      How can I help you today?
                    </h4>
                    <p className="text-sm text-zinc-500 dark:text-zinc-400 max-w-xs">
                      Ask me about your spending trends, budget status, or for
                ...
                {chatMessages.map((msg, i) => (
                  ...
                  >
                    <div
                      className={`max-w-[80%] p-4 rounded-2xl ${msg.role === "user" ? "bg-indigo-600 text-white" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white"}`}
                    >
                ...
                {isChatLoading && (
                  <div className="flex justify-start">
                    <div className="bg-zinc-100 dark:bg-zinc-800 p-4 rounded-2xl flex items-center gap-2">
              ...
              <form
                onSubmit={handleChat}
                className="p-6 border-t border-zinc-100 dark:border-zinc-800 shrink-0"
              >
                <div className="relative">
                  <input
                    type="text"
                    ...
                    className="w-full pl-6 pr-16 py-4 bg-zinc-100 dark:bg-zinc-800 border-none rounded-2xl text-sm focus:ring-2 focus:ring-indigo-500 transition-all dark:text-white"
                  />
                  <button
                    type="submit"
        ...
        {selectedStatDetails && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <p
                id="stat-title"
                className="text-xs font-bold text-zinc-500 uppercase tracking-[0.2em] mb-4 font-display"
              >
