      ...
      return onSnapshot(expensesQuery, (snapshot) => {
        ...
        groups.forEach(g => {
          ...
          if (totalSpent > g.maxBudget) {
            newAlerts.push({
              id: `over-budget-${g.id}`,
              message: `Group "${g.name}" is over its ${g.budgetType || 'total'} budget (${CURRENCY_SYMBOL}${totalSpent.toFixed(2)} / ${CURRENCY_SYMBOL}${g.maxBudget.toFixed(2)})`,
        ...
        try {
          handleFirestoreError(error, OperationType.LIST, `groups/${group.id}/expenses`);
        } catch (e) {
          // Silent if expected (e.g. group deleted)
        }
      });
    });
